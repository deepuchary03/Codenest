...
const IDE = () => {
  ...
  // Reset console height whenever it opens
  useEffect(() => {
    if (consoleOpen) {
  ...
  // Console resize dragging
  const handleConsoleResizeStart = (e) => {
    e.preventDefault();
    setIsDraggingConsole(true);
    setDragStartY(e.clientY);
    setDragStartHeight(consoleHeight);
  };
  ...
  useEffect(() => {
    const handleMouseMove = (e) => {
      if (isDraggingConsole) {
        ...
        const newHeight = Math.max(200, Math.min(600, dragStartHeight + delta));
  ...
  }, [isDraggingConsole, dragStartY, dragStartHeight]);
  ...
  return (
    <div className="h-screen flex flex-col bg-dark-bg">
      ...
      <div className="flex-1 flex overflow-hidden">
        {/* Sidebar - File Explorer & Test Cases - Fixed 14.29% (1/7) */}
        <div 
          style={{ width: '14.29%' }} 
              ...
              <div className="flex-1 overflow-y-auto">
                {currentProject?.files.map((file) => (
                  <div
                    ...
                    className={`w-full px-3 py-2 text-left text-sm flex items-center justify-between group hover:bg-dark-hover transition-colors cursor-pointer ${
        ...
        {/* Main Editor Area - Fixed 55.71% */}
        <div ref={editorAreaRef} style={{ width: '55.71%' }} className="flex flex-col relative">
          ...
          {testCases && (
            ...
            >
              <div className="px-4 py-2 flex items-start justify-between shrink-0">
                <div className="flex-1">
                  ...
                  <h2 className="text-lg font-bold mb-1 text-white">Problem Statement</h2>
                  <div className="text-sm text-gray-200 leading-relaxed overflow-y-auto" style={{ maxHeight: `${PROBLEM_HEIGHT - 70}px` }}>
          ...
          <div 
            ...
            style={{ 
              height: consoleOpen 
                ? `calc(100% - ${testCases ? PROBLEM_HEIGHT : 0}px - ${consoleHeight}px)` 
                : testCases 
                ? `calc(100% - ${PROBLEM_HEIGHT}px)` 
                : '100%'
            }}
          >
            {activeFile ? (
          ...
          {/* Console (Draggable Height) */}
          {consoleOpen && (
            <div 
              style={{ height: `${consoleHeight}px` }} 
              className="bg-dark-surface border-t border-dark-border flex flex-col relative overflow-hidden"
            >
              {/* Resize Handle */}
              <div
                onMouseDown={handleConsoleResizeStart}
                className="absolute top-0 left-0 right-0 h-2 cursor-ns-resize hover:bg-primary/50 transition-colors z-10 bg-dark-border/30"
                style={{ marginTop: '-1px' }}
              />
        ...
        {/* AI Panel - Fixed 30% - Always Visible */}
        <div 
          style={{ width: '30%' }} 
